...
## 📖 使用指南
1. 点击 **右上角 +**，进入重新设计的统一认证登录页，可一键切换移动/桌面模式（支持微信扫码登录）。
2. 登录成功后等待 5 秒，系统会自动提取 Cookies；如需立即写入，可在底部操作条中点击“手动保存认证信息”。
3. 回到主界面，在玻璃态卡片中管理账号，开启需要参与本次签到的用户，然后使用底部“扫描”按钮完成批量签到。
4. 如需手动录入 Cookies，使用底部“录入 Cookies”按钮。

> 拦截到的认证信息会保存在本地，可随时批量复用。

## 🆕 最新特性
- Apple 风格的浅色/深色主题，支持沉浸式系统栏与全新 SF 回退字重。
...
- 新增 `SessionRepositoryTest`，可通过 `./gradlew connectedAndroidTest` 验证选人持久化逻辑。
